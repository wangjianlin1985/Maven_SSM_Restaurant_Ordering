...
public interface DinnerNoticeDao {
    ...
    //根据id查询所有小票信息
    @Select("select *from dinner_notice  where order_id=#{id} ")
    List<DinnerNotice> getAllDinnerNoticeByOID(Integer id);
    ...
    //添加小票
    @Insert("INSERT INTO `ssm_dinner_manage`.`dinner_notice`( `menu_name`, `order_id`, `menunum`, `remark`, `menuprice`) VALUES ( #{menuName} , #{orderId} , #{menunum} , #{remark} , #{menuprice} );")
