...
public interface DiningTableDao {
    ...
    // 修改
    @Update("UPDATE `ssm_dinner_manage`.`dining_table` SET `table_num` =#{tableNum} , `table_pernum` = #{tablePernum}  WHERE `id` = #{id} ;")
    ...
    @Insert("INSERT INTO `ssm_dinner_manage`.`dining_table`(`table_num`, `table_pernum`, `isno`) VALUES ( #{tableNum} , #{tablePernum} , #{isno} );")
    ...
    // 查询所有空闲餐桌
    @Select("select *from dining_table where isno=1")
    List<DiningTable> getdiningUseTable();
